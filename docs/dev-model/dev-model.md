...
本文主要介绍一种全新的前后端合作开发模式。在介绍这种模式之前咋们先来看看前后端的合作模式的演变。
...
前端开发完页面，输出给后端。后端拿到页面拼模板，然后再渲染输出。但随着前端业务复杂性逐渐增加，这种模式存在一个隐藏bug。**后端在拼模板时，不能确保输出的页面结构与前端保持一致**。为了规避这个问题，聪明的开发人员就给前端同学建议，**使用后端开发环境开发前端页面**。

![model-2](./imgs/dev-model-2.png)

前端直接在本地部署后端开发环境，只负责 views 开发。再基于代码托管工具 git，实现前后端的合作。此时还是以后端为主导，维护性依然很高。
...
这种模式的核心：**客户端渲染 + 接口**。由于基于客户端渲染，对浏览器的 SEO 不是很友好。虽然可以通过 SRR 来解决，但是 SRR 也存在局限性。关于 SRR 感兴趣的同学可自行查阅相关的资料。本文主要探讨传统传统的多页面应用构建的优化。

基于前端工程化，要让传统多页面应用也支持前后端完全分离。还需要做一件事，前端架手架需要与与后端使用相同的模板渲染引擎。前端编写好后，直接输出模板给后端使用。为此自己也构建一个 fes 脚手架。也构建了几个项目，体验是有所提升，但同时也暴露一个可以接受的问题。

![model-4](./imgs/dev-model-4.png)

在与后端合作时，需要提前约定好模板数据变量，一般会以文档形式进行说明。而这种合作方式工作起来不是轻松。因为在开发之前需要与后端约定好模板数据变量，一般会以文档形式进行说明。如果后端更新了数据，没同步更新文档，就会存在数据不一致的问题。对开发效率大打折扣，与传统的后端重新拼接模板相比优势不是很明显。秉着 geek 的精神，就想能不能**将模板数据以接口的形式提供给前端**。这样上述问题不就迎刃而解了么。

![model-5](./imgs/dev-model-5.png)

于是就对fes进行改造。让其支持模板数据接口的配置，在渲染之前会根据配置接口来拉起数据，再进行渲染输出。同时还提供了接口数据适配功能，能让客户端更好地控制数据结构。
```js
mockConfig: {
  // 访问路径作为key
  '/index': {
...
`http://127.0.0.1:3001/mockdata/index`接口返回的数据如下：
