...
再使用 fes 构建了几个项目，虽然在开发体验、开效率上都得到了很大地提升。但在与后端合作时，需要提前约定好模板数据变量，一般会以文档形式进行说明。而这种合作方式工作起来不是轻松。因为在开发之前需要与后端约定好模板数据变量，一般会以文档形式进行说明。如果后端更新了数据，没同步更新文档，就会存在数据不一致的问题。对开发效率大打折扣，与传统的后端重新拼接模板相比优势不是很明显。秉着 geek 的精神，就想能不能**将模板数据以接口的形式提供给前端**。这样上述问题不就迎刃而解了么。
...
以`index`为例进行讲解，模板数据接口为：`http://127.0.0.1:3001/mockdata/index`。接口返回的数据如下：
...
```
其中`/mockdata`是前缀，在 fes 中对模板数据接口进行配置
