...
const RasterFactory = require('./factories/raster');
const VectorFactory = require('./factories/vector');
	...
	root.walkRules((rule) => {
      ...
      if(/^~/.test(url)){
        Object.keys(alias).forEach((item) => {
          ...
          if(url !== tempUrl && /\/sprite\//.test(url)){
 ...
 * @return
 */
function updateRule(rule, token, image, dpr) {
  ...
  if(dpr && typeof dpr === 'function'){
	...
	token.cloneAfter({
		...
		value: dpr ? `${posX} ${posY}` : `${posX}px ${posY}px`
	}).cloneAfter({
		prop: 'background-size',
		value: dpr ? `${sizeX} ${sizeY}` : `${sizeX}px ${sizeY}px`
	});
}
 ...
 * @return {Function}
 */
function maskGroup(toggle = false) {
 ...
 * @return {String}
 */
function makeSpritesheetPath(opts, { groups, extension }) {
